...
if(_debug) fprintf(stderr,"CALLING --- vx_getcoord_private (enhanced %d)\n",enhanced);
  ...
  if(enhanced == True) {
      ...
      if(cvmhlabn_debug) { fprintf(stderr," cvmh surface -- %lf\n", surface); }
      if (surface < -90000.0) {
        surface_nodata_count++;
	return(1);
      }

      entry->depth = p0_NO_DATA_VALUE;
      switch (vx_zmode) {
      case VX_ZMODE_ELEV:
	break;
      case VX_ZMODE_DEPTH:
	...
	if (entry.topo > entry.mtop) {
	  ...
	  *surface = entry.topo - ELEV_EPSILON;
        ...
        if(TARGET_COOR_UTM2 != -99999.0) {  // only the first time 
          ...
          max_iter = (tmp < 0)?0 : tmp+1;
          } else {
            max_iter=0;
        }

if(cvmhlabn_debug) fprintf(stderr," new max iter..%d (%lf : %lf)\n", max_iter, COOR_UTM2, *surface);
...
if(cvmhlabn_debug) fprintf(stderr," SURFACE: LOOPING in here %lf\n", entry.coor[2]);
	  if (num_iter > max_iter) { 
	    *surface = p0_NO_DATA_VALUE;
	    flag = 1;
if(cvmhlabn_debug) fprintf(stderr,"SURFACE: over max_iter set  to NO_DATA_VALUE\n");
	  }
	  num_iter = num_iter + 1;
...
if(cvmhlabn_debug) fprintf(stderr,"SURFACE: LOOPING checked %lf\n", entry.coor[2]);
	}
if(cvmhlabn_debug) fprintf(stderr,"SURFACE: LOOPING is over %lf\n", entry.coor[2]);
      } else {
	do_bkg = True;
      }
    }
...
if(_debug)  fprintf(stderr,"DONE -- vx_getsurface_private surface found=%lf\n", *surface);

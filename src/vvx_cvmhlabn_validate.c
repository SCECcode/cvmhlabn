 ...
 *
 *
 *  ./vx_cvmhlabn_validate -z elev -f CVMHB-San-Gabriel-Basin.dat
 *
 */
        ...
        while(rc==0) {
              ...
              if(rc == 0) {
                ...
                if(_compare_double(ret.vs, dat.vs) ||
                   ...
                   printf("VALIDATE: BAD,  %lf %lf %lf\n",pt.longitude, pt.latitude, pt.depth);
              }
          rc=_next_datfile(fp, &dat);
        }

        fprintf(stderr,"VALIDATE: %d mismatch out of %d \n", mcount, tcount);
        fprintf(stderr,"VALIDATE:  BAD surface count %d\n", bad_surface_count);
